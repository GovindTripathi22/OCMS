...
// ---------------------------------------------------------------------------
export const CURATED_PALETTES: Record<string, string[]> = {
    // Technology / digital
...
export function generateHarmonyPalette(hue: number): string[] {
    return [
        hslToHex(hue, 10, 96),
        hslToHex(hue, 70, 45),
...
export function extractColorsFromHtmlAndCss(content: string): string[] {
    const colorCount = new Map<string, number>();

    // 1. Semantic CSS custom properties (high priority)
    const cssVarRegex = /--[a-zA-Z0-9_-]*(?:color|bg|background|primary|secondary|accent|text|theme)[a-zA-Z0-9_-]*\s*:\s*(#[0-9a-fA-F]{3,8}|rgba?\([^)]+\))/gi;
    ...
    // 2. Generic hex colors
    const hexRegex = /#(?:[0-9a-fA-F]{6}|[0-9a-fA-F]{3})\b/g;
    while ((match = hexRegex.exec(content)) !== null) {
        const hex = normalizeColorToHex(match[0]);
        if (hex) {
...
export function matchPalette(description: string): string[] {
